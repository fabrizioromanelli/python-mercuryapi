...
MERCURYAPI_JAVA_SRCS= ${MERCURYAPI_JAVA_SRCDIR}/ClientHttpRequest.java \
 ...
 ${MERCURYAPI_JAVA_SRCDIR}/LlrpTransport.java\
...
$(MERCURYAPI_JAVA_BUILD)/%.class : $(MERCURYAPI_JAVA_DIR)/mercuryapi_nb/src/$(MERCURYAPI_JAVA_SUBDIR)/%.java
	javac -target 1.6 -source 1.6 -d ${BUILD} -cp $(MERCURYAPI_JAVA_NETBEANS_SRCDIR) $<
...
$(BUILD)/demo.jar : $(BUILD)/SavedConfig.class $(BUILD)/MultiProtocol.class $(BUILD)/multireadasync.class
	(cd $(BUILD); jar -cf $(notdir $@) samples/demo*.class samples/read*.class samples/locktag*.class samples/serial*.class samples/commandtime*.class samples/filter*.class samples/multireadasync*.class samples/Block*.class samples/LicenseKey*.class samples/SavedConfig*.class samples/MultiProtocol*.class)

$(BUILD)/%.class : $(MERCURYAPI_JAVA_DIR)/samples_nb/src/samples/%.java $(MERCURYAPI_JARFILE)
	javac -target 1.6 -source 1.6 -d ${BUILD} -cp $(MERCURYAPI_JARFILE) $<
...
$(BUILD)/doc/index.html: $(MERCURYAPI_JAVA_SRCS) autoparams_java
	mkdir -p $(BUILD)/doc
	javadoc -public -sourcepath $(MERCURYAPI_JAVA_NETBEANS_SRCDIR) -d $(BUILD)/doc com.thingmagic
